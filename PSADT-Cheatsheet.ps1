...
Invoke-HKCURegistrySettingsForAllUsers -RegistrySettings {Set-RegistryKey -Key 'HKEY_CURRENT_USER\Software\UNIT4\ReportEngine\Login\WebService\AgressoAuthenticator' -Name 'UserName' -Value $Username  -Type String -SID $UserProfile.SID}
...
##***Example Shortcut***
New-Shortcut -Path "$envCommonStartMenuPrograms\Camera Base.lnk" -TargetPath "C:\Databases\Camerabase\CameraBase.mdb" -IconLocation "C:\Databases\Camerabase\CameraBase.mdb" -Description 'Camera Base' -WorkingDirectory "$envHomeDrive\$envHomePath"
